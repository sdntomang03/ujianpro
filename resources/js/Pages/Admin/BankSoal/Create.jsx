...
export default function Create({ auth, kategori }) {
    ...
    const { data, setData, post, processing, errors } = useForm({
        ...
        pasangan: [{ kiri: 'Premis 1', kanan: 'Respon 1' }, { kiri: 'Premis 2', kanan: 'Respon 2' }],
        pernyataan_bs: [
    ...
    useEffect(() => {
        // Reset state parsial jika tipe berubah agar UI bersih
            ...
            return (
                <div className="space-y-4">
                    <div className="flex gap-3 px-3">
                        <div className="flex-1 text-xs font-bold text-slate-400 uppercase">Pernyataan</div>
                        ...
                        <div key={index} className="flex items-center gap-3 bg-white p-3 rounded-xl border border-slate-200 shadow-sm transition-all focus-within:border-indigo-500 focus-within:ring-1">
                            <div className="flex-1">
                                <textarea value={item.teks} onChange={e => {
                                    ...
                                    }} rows="2" placeholder="Ketik pernyataan..." className="w-full border-0 focus:ring-0 text-sm p-2 resize-none" required></textarea>
                            </div>
                            <div className="w-32 flex justify-center gap-2 border-l border-slate-100 pl-3">
                                <label className={`cursor-pointer px-3 py-1.5 rounded-lg text-sm font-bold transition ${item.kunci === 'Benar' ? 'bg-emerald-500 text-white' : 'bg-slate-100 text-slate-500 hover:bg-slate-200'}`}>
                                ...
                                <label className={`cursor-pointer px-3 py-1.5 rounded-lg text-sm font-bold transition ${item.kunci === 'Salah' ? 'bg-rose-500 text-white' : 'bg-slate-100 text-slate-500 hover:bg-slate-200'}`}>
                ...
                <div className="space-y-4">
                    ...
                    {data.pasangan.map((pair, index) => (
                        <div key={index} className="flex gap-4 items-center">
                            ...
                            <button type="button" onClick={() => { if(data.pasangan.length <= 2) return alert('Minimal 2 pasang!'); setData('pasangan', data.pasangan.filter((_, i) => i !== index)); }} className="text-rose-500 p-2"><svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" /></svg></button>
                        </div>
                    ))}
                    <button type="button" onClick={() => setData('pasangan', [...data.pasangan, { kiri: '', kanan: '' }])} className="w-full py-2 border border-dashed rounded-xl text-sm font-bold text-slate-500 hover:bg-slate-50 transition">+ Tambah Baris</button>
                </div>
            );
        }
    };
    ...
    return (
        <AuthenticatedLayout header={<h2 className="font-bold text-2xl text-slate-800 tracking-tight">Question Builder Pro</h2>}>
            ...
            <style>{`.quill-editor .ql-container { min-height: 150px; border-bottom-radius: 0.75rem; font-size: 15px; }`}</style>
                ...
                <div className="lg:col-span-2 space-y-6">
                    ...
                    <div className="bg-white rounded-3xl shadow-sm border border-slate-100 overflow-hidden">
                        <div className="bg-slate-50/80 px-6 py-4 border-b border-slate-100 flex items-center gap-3">
                            ...
                            <h3 className="font-bold text-slate-800">Konten Pertanyaan Utama</h3>
                        </div>
                        <div className="p-6">
                            <div className="quill-editor mb-6">
                                <ReactQuill theme="snow" value={data.pertanyaan} onChange={c => setData('pertanyaan', c)} placeholder="Ketikkan teks pertanyaan atau instruksi di sini..." />
                                {errors.pertanyaan && <p className="text-rose-500 text-xs mt-2">{errors.pertanyaan}</p>}
                            </div>
                            <div>
                                <label className="text-xs font-bold text-slate-500 uppercase mb-2 block">Lampiran Gambar (Opsional)</label>
                                {!imagePreview ? (
                                    <div onClick={() => fileInputRef.current.click()} className="border-2 border-dashed border-slate-300 rounded-2xl p-8 flex flex-col items-center justify-center text-slate-500 hover:bg-slate-50 cursor-pointer">
                                        <p className="text-sm font-semibold">Klik untuk upload gambar</p>
                                    </div>
                                ) : (
                                    <div className="relative border border-slate-200 rounded-2xl p-2 bg-slate-50 inline-block">
                                        <img src={imagePreview} className="h-40 w-auto rounded-xl" />
                                        <button type="button" onClick={() => {setData('gambar', null); setImagePreview(null);}} className="absolute -top-3 -right-3 bg-rose-500 text-white rounded-full p-1.5 shadow-lg">X</button>
                                    </div>
                                )}
                    ...
                    <div className="bg-white rounded-3xl shadow-sm border border-slate-100 overflow-hidden">
                        <div className="bg-slate-50/80 px-6 py-4 border-b border-slate-100 flex items-center gap-3">
                            ...
                            <h3 className="font-bold text-slate-800">Opsi & Kunci Jawaban</h3>
                        </div>
                    ...
                    <div className="bg-white rounded-3xl shadow-sm border border-slate-100 overflow-hidden sticky top-24">
                        ...
                        <div className="p-6 space-y-5">
                            ...
                            <div>
                                ...
                                <input type="number" value={data.bobot_nilai} onChange={e => setData('bobot_nilai', e.target.value)} className="mt-2 w-full border-slate-200 rounded-xl focus:ring-indigo-500 text-sm font-bold" required />
                            </div>
                            <button type="submit" disabled={processing} className="w-full py-3.5 bg-indigo-600 text-white font-black rounded-xl shadow-lg hover:bg-indigo-700">Simpan Soal</button>
                        </div>
                    </div>
                </div>
            </form>
