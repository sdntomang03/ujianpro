...
export default function Edit({ auth, soal, kategori }) {
    ...
    const initData = () => {
        ...
        let kunci_kompleks = [];
        let kunci_isian = '';
        let pasangan = [{ kiri: '', kanan: '' }, { kiri: '', kanan: '' }];
        let pernyataan_bs = [{ teks: '', kunci: 'Benar' }, { teks: '', kunci: 'Salah' }];

        const dbOpsi = soal.opsis || [];

        if (soal.tipe_soal === 'pg' || soal.tipe_soal === 'survei') {
            if (dbOpsi.length > 0) {
                opsi = dbOpsi.map(o => o.teks_opsi);
                const trueIdx = dbOpsi.findIndex(o => o.is_correct);
                kunci_jawaban = trueIdx !== -1 ? trueIdx : 0;
            }
        } else if (soal.tipe_soal === 'pg_kompleks') {
            if (dbOpsi.length > 0) {
                opsi = dbOpsi.map(o => o.teks_opsi);
                dbOpsi.forEach((o, idx) => {
                    if (o.is_correct) kunci_kompleks.push(idx);
                });
            }
        } else if (soal.tipe_soal === 'isian') {
            kunci_isian = dbOpsi.filter(o => o.is_correct).map(o => o.teks_opsi).join(', ');
        } else if (soal.tipe_soal === 'benar_salah') {
            if (dbOpsi.length > 0) {
                pernyataan_bs = dbOpsi.map(o => ({
                    teks: o.teks_opsi,
                    kunci: o.is_correct ? 'Benar' : 'Salah'
                }));
            }
            ...
            if (dbOpsi.length > 0) {
                ...
                for(let i=0; i < Math.max(lefts.length, rights.length); i++) {
                    tempPasangan.push({
                        kiri: lefts[i] ? lefts[i].teks_opsi : '',
                        kanan: rights[i] ? rights[i].teks_opsi : ''
                    });
                }
                if (tempPasangan.length > 0) pasangan = tempPasangan;
            }
        }

        return { opsi, kunci_jawaban, kunci_kompleks, kunci_isian, pasangan, pernyataan_bs };
    };

    const initialParsedState = initData();

    // Inisialisasi Form Inertia
    const { data, setData, post, processing, errors } = useForm({
        _method: 'PUT', // Wajib untuk Laravel jika form berpotensi mengirim file gambar pada aksi Update
    ...
    useEffect(() => {
        ...
        switch(data.tipe_soal) {
            case 'pg': setData('kunci_jawaban', 0); break;
            case 'pg_kompleks': setData('kunci_kompleks', []); break;
            case 'benar_salah': setData('pernyataan_bs', [{ teks: '', kunci: 'Benar' }, { teks: '', kunci: 'Salah' }]); break;
            case 'isian': setData('kunci_isian', ''); break;
        }
    }, [data.tipe_soal]);
                            ...
                            <div>
                                ...
                                {!imagePreview ? (
                                    <div onClick={() => fileInputRef.current.click()} className="border-2 border-dashed border-slate-300 rounded-2xl p-8 flex flex-col items-center justify-center text-slate-500 hover:bg-slate-50 cursor-pointer transition-colors group">
                                        ...
                                        <p className="text-sm font-semibold">Klik untuk upload gambar pengganti</p>
                                    </div>
                                ) : (
                ...
                <div className="space-y-6">
                    <div className="bg-white rounded-3xl shadow-sm border border-slate-100 overflow-hidden sticky top-24">
                        ...
                        <div className="p-6 space-y-5">
                            ...
                            <div className="pt-2">
                                <button type="submit" disabled={processing} className="w-full py-3.5 bg-indigo-600 text-white font-black rounded-xl shadow-lg hover:bg-indigo-700 transition disabled:opacity-50 flex justify-center items-center gap-2">
